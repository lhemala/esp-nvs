...
fn roundtrip_csv_file(csv_path: &str, partition_size: usize) {
    ...
    let binary1 = original_partition.generate_partition(partition_size).unwrap();
    ...
    for (i, (e1, e2)) in parsed1.entries.iter().zip(parsed2.entries.iter()).enumerate() {
        assert_eq!(e1.namespace, e2.namespace, "namespace mismatch at entry {i}");
    ...
    for (i, (e2, e3)) in parsed2.entries.iter().zip(parsed3.entries.iter()).enumerate() {
        assert_eq!(
...
fn test_roundtrip_variable_len() {
    ...
    assert_entry_content(&parsed, 1, &EntryContent::Data(DataValue::Binary(blob_data)));

    let lorem_string = fs::read_to_string("tests/assets/lorem_string.txt").unwrap();
    assert_entry_content(&parsed, 2, &EntryContent::Data(DataValue::String(lorem_string)));
    ...
    assert_entry_content(&parsed, 4, &EntryContent::Data(DataValue::Binary(multi_blob_data)));
}
