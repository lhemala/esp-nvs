...
pub(crate) fn write_csv_content(partition: NvsPartition) -> Result<String, Error> {
    ...
    let bytes = wtr.into_inner().map_err(|e| Error::IoError(e.into_error()))?;
    String::from_utf8(bytes).map_err(|e| Error::InvalidValue(format!("CSV output is not valid UTF-8: {}", e)))
}

fn write_records<W: std::io::Write>(wtr: &mut csv::Writer<W>, partition: NvsPartition) -> Result<(), Error> {
