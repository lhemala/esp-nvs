...
impl NvsEntry {
    ...
    pub fn new_file(namespace: String, key: String, encoding: FileEncoding, file_path: PathBuf) -> Self {
        Self {
            namespace,
            key,
            content: EntryContent::File { encoding, file_path },
        }
    }
    ...
    /// Set the content to inline data.
    ///
    /// Convenience method for `set_content(EntryContent::Data(value))`.
    pub fn set_data(&mut self, value: DataValue) {
        self.content = EntryContent::Data(value);
    }

    /// Set the content to a file reference.
    ///
    /// Convenience method for `set_content(EntryContent::File { .. })`.
    pub fn set_file(&mut self, encoding: FileEncoding, file_path: PathBuf) {
        self.content = EntryContent::File { encoding, file_path };
    }
}
